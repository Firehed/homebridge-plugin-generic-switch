...
class GenericSwitch {
  ...
  constructor(log, config) {
    this.createServices = () => {
      ...
      infoService.setCharacteristic(Characteristic.Manufacturer, this.manufacturer).setCharacteristic(Characteristic.Model, this.model).setCharacteristic(Characteristic.SerialNumber, this.sn);
    ...
    this.getPower = cb => {
      ...
      fetch(this.host + this.api.get.route).then(res => {
        if (!res.ok) {
    ...
    this.setPower = (on, cb) => {
      ...
      fetch(this.host + this.api.set.route + '?' + this.api.set.param + '=' + state, { method: "POST" }).then(_ => {
        this.lastState = on;
        this.lastChecked = Date.now();
        cb();
      });
